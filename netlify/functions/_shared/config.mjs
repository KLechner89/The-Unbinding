...
export function postmarkToken() {
  // Reuses the Postmark server token already configured for the Netlify Email
  // Extension, unless a dedicated POSTMARK_SERVER_TOKEN is set.
  if (env('POSTMARK_SERVER_TOKEN')) return env('POSTMARK_SERVER_TOKEN');
  if (env('NETLIFY_EMAILS_PROVIDER').toLowerCase() === 'postmark') return env('NETLIFY_EMAILS_PROVIDER_API_KEY');
  return '';
}
...
export function settings() {
  ...
  return {
    ...
    conflictCalendarIds: [...new Set([
      ...
      ...(RULES.additionalConflictCalendarIds || []),
    ])].filter(Boolean),
    email: {
      token: postmarkToken(),
...
export function readiness() {
  ...
  if (!postmarkToken()) missing.push('Postmark server token (NETLIFY_EMAILS_PROVIDER_API_KEY or POSTMARK_SERVER_TOKEN)');
